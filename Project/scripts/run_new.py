...
def update(t_idx):
    ...
    dt_text = "    $\Delta t^*= {:.1f}\cdot 10^{{{:.0f}}}$".format(mantisse_dt, -3)
    axs[0].set_title(title_text + dt_text, fontsize=title_fontsize)
...
if __name__ == "__main__":
    ...
    html_ref = "../anim/case_5.html"  # copy and create new html with a different path to images
    ...
    title_text += "    $h^* = {:.3f}$".format(sim.h)
    base10power_dt = np.floor(np.log10(sim.dt))
    mantisse_dt = sim.dt / np.power(10, base10power_dt)
    dt_text = "    $\Delta t^*= {:.1f}\cdot 10^{{{:.0f}}}$".format(mantisse_dt, base10power_dt)
    axs[0].set_title(title_text + dt_text, fontsize=title_fontsize)
